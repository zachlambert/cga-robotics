...
class GeometricProduct:
    def write_individual(self, G, op1, op2, available, f_h, f_cpp):
        expression = (op1.expression("lhs") * op2.expression("rhs")).expand()
        ret = find_return_struct(G, expression, available)
        if ret is None:
    ...
    def write(self, G, structs, available, f_h, f_cpp):
        ...
        for i in range(len(structs)):
            ...
            for j in range(i, len(structs)):
                ...
                self.write_individual(G, structs[i], structs[j], available, f_h, f_cpp)
